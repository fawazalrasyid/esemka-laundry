...
$query = mysqli_query($conn, "SELECT COUNT(id_transaksi) as jumlah_transaksi FROM transaksi WHERE outlet_id = '$id_outlet'");
...
$query4 = mysqli_query($conn, "SELECT SUM(total_harga) as total_penghasilan FROM detail_transaksi INNER JOIN transaksi ON transaksi.id_transaksi = detail_transaksi.id_transaksi WHERE status_bayar = 'dibayar' AND transaksi.outlet_id = '$id_outlet'");
$total_penghasilan = mysqli_fetch_assoc($query4);

$query5 = mysqli_query($conn, "SELECT MONTHNAME(tgl_pembayaran) as bulan, SUM(total_harga) as penghasilan_bulan FROM detail_transaksi INNER JOIN transaksi ON transaksi.id_transaksi = detail_transaksi.id_transaksi WHERE status_bayar = 'dibayar' AND outlet_id = '$id_outlet' AND YEAR(tgl_pembayaran) = YEAR(NOW()) GROUP BY MONTH(tgl_pembayaran)");
